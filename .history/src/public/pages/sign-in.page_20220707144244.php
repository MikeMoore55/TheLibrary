...
<?php
    ...
    if($_SERVER["REQUEST_METHOD"]== "POST"){
        ...
        $sql = "SELECT 'username', 'user_age', 'user_password', 'user_type' FROM user_info WHERE username = $username";

        $result = $conn->query($sql);

        while($row = $result->fetch_assoc()) {
            echo "id: " . $row["user_id"]. " - Name: " . $row["username"]. " " . $row["user_age"]. " ". $row["user_type"]. "<br>";
        }
